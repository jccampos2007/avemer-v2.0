    ...
    <form id="form_banco" action="<?php echo BASE_URL; ?>banco/<?php echo ($is_edit) ? 'update/' . htmlspecialchars($banco_data['id']) : 'store'; ?>" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            <div>
                ...
                <input type="text" id="nombre" name="nombre" value="<?php echo htmlspecialchars($banco_data['nombre'] ?? ''); ?>" class="input-form focus:outline-none focus:shadow-outline" required>
            </div>
        </div>
