...
$(document).ready(function () {
    ...
    if (eventoTable.length) {
            ...
            "buttons": [
                {
                    ...
                    exportOptions: {
                        columns: [1, 2, 3] // Exportar únicamente Duración, Nombre y Descripción
                    },
                    ...
                    "render": function (data, type, row) {
                        ...
                        if (typeof EVENTO_PERMISSIONS !== 'undefined') {
                            if (EVENTO_PERMISSIONS.modificar) {
                                actions += `<a href="evento/edit/${row[0]}" class="btn-action btn-action-edit" title="Editar"><i class="fas fa-edit"></i></a>`;
                            }
                            if (EVENTO_PERMISSIONS.eliminar) {
                                actions += `<a href="evento/delete/${row[0]}" class="btn-action btn-action-delete" title="Eliminar"><i class="fas fa-trash-alt"></i></a>`;
                            }
                        } else {
                            // Fallback por si la variable no está definida
                            actions += `
                                <a href="evento/edit/${row[0]}" class="btn-action btn-action-edit" title="Editar"><i class="fas fa-edit"></i></a>
                                <a href="evento/delete/${row[0]}" class="btn-action btn-action-delete" title="Eliminar"><i class="fas fa-trash-alt"></i></a>
                            `;
                        }
        ...
        eventoTable.on("click", ".btn-action-delete", function (e) {
            e.preventDefault();
